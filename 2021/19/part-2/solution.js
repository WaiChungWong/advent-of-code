...
function findSolution(data) {
  ...
  for (let i = 0; i < scannerPositions.length; i++) {
    const position1 = scannerPositions[i];

    for (let j = i + 1; j < scannerPositions.length; j++) {
      const position2 = scannerPositions[j];

      const manhattanDistance = position1.map((v, vi) => v - position2[vi]).reduce((acc, v) => acc + v, 0);
